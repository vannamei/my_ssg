...
export default class GetTemplates {
  private readonly ext: TemplateExtension;
  public readonly HTMLWebpackPluginList: HTMLWebpackPlugin[];

  constructor(private readonly option: GetTemplatesOption) {
    switch (option.engine) {
      ...
      case "none":
        ...
        break;
      default:
        this.ext = option.engine;
        break;
    }
    const fileNames = this.getFileNames();
    this.HTMLWebpackPluginList = fileNames.map(this.toPluginList.bind(this));
  ...
  private getFileNames(this: GetTemplates): string[] {
    ...
    if (!fileNames.length) {
      ...
      console.log(
        `\x1b[35mFound ${fileNames.length} template files.\x1b[0m`,
        fileNames
      );
      return fileNames;
    }
